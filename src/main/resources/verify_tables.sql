-- Verify that all tables exist and have the correct structure

-- Check if all required tables exist
\dt

-- Check the structure of each table
\d users
\d trips
\d expenses
\d activities
\d countries
\d states
\d destinations
\d trip_history
\d expense_breakdown

-- Verify foreign key relationships by checking constraints
SELECT 
    tc.table_name, 
    tc.constraint_name, 
    tc.constraint_type,
    kcu.column_name, 
    ccu.table_name AS foreign_table_name,
    ccu.column_name AS foreign_column_name 
FROM 
    information_schema.table_constraints AS tc 
    JOIN information_schema.key_column_usage AS kcu
      ON tc.constraint_name = kcu.constraint_name
      AND tc.table_schema = kcu.table_schema
    JOIN information_schema.constraint_column_usage AS ccu
      ON ccu.constraint_name = tc.constraint_name
      AND ccu.table_schema = tc.table_schema
WHERE 
    tc.constraint_type = 'FOREIGN KEY' 
    AND tc.table_name IN ('users', 'trips', 'expenses', 'activities', 'countries', 'states', 'destinations', 'trip_history', 'expense_breakdown')
ORDER BY 
    tc.table_name;

-- Verify indexes
SELECT 
    tablename, 
    indexname, 
    indexdef 
FROM 
    pg_indexes 
WHERE 
    tablename IN ('users', 'trips', 'expenses', 'activities', 'countries', 'states', 'destinations', 'trip_history', 'expense_breakdown')
ORDER BY 
    tablename, 
    indexname;

-- Check sample data
SELECT COUNT(*) as user_count FROM users;
SELECT COUNT(*) as country_count FROM countries;
SELECT COUNT(*) as state_count FROM states;
SELECT COUNT(*) as destination_count FROM destinations;
SELECT COUNT(*) as trip_count FROM trips;
SELECT COUNT(*) as expense_count FROM expenses;
SELECT COUNT(*) as activity_count FROM activities;
SELECT COUNT(*) as trip_history_count FROM trip_history;
SELECT COUNT(*) as expense_breakdown_count FROM expense_breakdown;